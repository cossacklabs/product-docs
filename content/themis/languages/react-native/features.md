...
Generate a long strong key and use `secureCellSealWithSymmetricKeyEncrypt64` function to encrypt `plaintext` data with optional `context`. 
...
secureCellSealWithPassphraseDecrypt64(passphrase, encryptedDataBase64, context)
  .then((decrypted) => {
...
Encrypt the data using the `secureCellTokenProtectEncrypt64` method:
