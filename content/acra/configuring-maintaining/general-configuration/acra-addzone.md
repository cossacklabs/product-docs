...
* `--vault_tls_client_auth=<mode>`
  ...
  * `4` — (default) request and validate client certificate.
  ...
  These values correspond to [crypto.tls.ClientAuthType](https://golang.org/pkg/crypto/tls/#ClientAuthType).
  If not specified, acra-addzone uses value from `--tls_auth` flag.
...
* `--vault_tls_client_ca=<filename>`
  ...
  Path to acra-addzone TLS certificate's CA certificate for Vault certificate validation (AcraServer works as "client" when communicating with Vault).
  Empty by default.
  If not specified, acra-addzone uses value from `--tls_ca` flag.
...
* `--vault_tls_client_cert=<filename>`
  ...
  Path to acra-addzone TLS certificate presented to Vault (AcraServer works as "client" when communicating with Vault).
  Empty by default.
  If not specified, acra-addzone uses value from `--tls_cert` flag.
